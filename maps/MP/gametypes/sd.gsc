...
spawnSpectator(origin, angles)
{
	...
	// START: Insert auto-ready logic for spectators:
	self thread spectatorReadyup();
	// END
...
updateGametypeCvars()
{
	level endon("PAMEnabled");

	for(;;)
	{
		// WORM PAM Enable Check
		pamenable = getCvarint("svr_pamenable");
		if (level.pamenable != pamenable)
		{
			level.pamenable = pamenable;
			//iprintln("^2PAM has been turned ON!");
			//iprintln("^2MAP_ROTATE is required");

			maps\mp\gametypes\_pam_utilities::StartPAMUO("enable");

			level notify("PAMEnabled");

		}
		// End Worm PAM Enable Check
...
autoAssignTeam(entity)
{
    numAllies = countTeamPlayers("allies");
    numAxis = countTeamPlayers("axis");
    if (numAllies <= numAxis)
        team = "allies";
    else
        team = "axis";
    entity.pers["team"] = team;
    entity.sessionteam = team;
    entity iprintlnbold("^7You have been auto-assigned to " + team);
    entity thread maps\mp\gametypes\_pam_readyup::readyup(entity getEntityNumber());

}

spectatorReadyup()
{
    // Read spectator auto-ready time from the cvar (defined in _pam_teams.gsc)
    autoSpecTime = getCvarFloat("g_autoSpectatorTime");
    if (!isdefined(autoSpecTime) || autoSpecTime <= 0)
        autoSpecTime = 30;
    specReadyHud = newClientHudElem(self);
    specReadyHud.x = 320;
    specReadyHud.y = 200;
    specReadyHud.alignX = "center";
    specReadyHud.alignY = "middle";
    specReadyHud.fontScale = 1.2;
    startTime = getTime();
    prevDisplay = roundUp(autoSpecTime);
    while(self.pers["team"] == "spectator")
    {
        elapsed = (getTime() - startTime) / 1000.0;
        remaining = autoSpecTime - elapsed;
        if (remaining <= 0)
            break;
        display = roundUp(remaining);
        if (display != prevDisplay)
        {
            specReadyHud setText("Auto joining in: " + display + " seconds");
            prevDisplay = display;
        }
        wait 0.1;
    }
    if (isdefined(specReadyHud))
        specReadyHud destroy();
    if (self.pers["team"] == "spectator")
    {
        autoAssignTeam(self);
    }
