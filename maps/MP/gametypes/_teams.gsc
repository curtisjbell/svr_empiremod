...
initGlobalCvars()
{
	...
	setCvar("scr_friendlyfire", level.friendlyfire, false);
	...
	// PAM UO Auto-Ready
	level.g_AutoReadyTime = getCvar("g_AutoReadyTime");
	if(level.g_AutoReadyTime == "")
		level.g_AutoReadyTime = "30";
	setCvar("g_AutoReadyTime", level.g_AutoReadyTime);
	makeCvarServerInfo("g_AutoReadyTime", "30");

	level.g_AutoSpectatorTime = getCvar("g_AutoSpectatorTime");
	if(level.g_AutoSpectatorTime == "")
		level.g_AutoSpectatorTime = "30";
	setCvar("g_AutoSpectatorTime", level.g_AutoSpectatorTime);
	makeCvarServerInfo("g_AutoSpectatorTime", "30");

	// Server Branding / Customization
	level.ui_BrandServerName = getCvar("ui_BrandServerName");
	if(level.ui_BrandServerName == "")
		level.ui_BrandServerName = "^1~^3empire";
	setCvar("ui_BrandServerName", level.ui_BrandServerName);
	makeCvarServerInfo("ui_BrandServerName", "^1~^3empire");

	level.ui_BrandColorPrimary = getCvar("ui_BrandColorPrimary");
	if(level.ui_BrandColorPrimary == "")
		level.ui_BrandColorPrimary = "3";
	setCvar("ui_BrandColorPrimary", level.ui_BrandColorPrimary);
	makeCvarServerInfo("ui_BrandColorPrimary", "3");

	level.ui_BrandColorSecondary = getCvar("ui_BrandColorSecondary");
	if(level.ui_BrandColorSecondary == "")
		level.ui_BrandColorSecondary = "1";
	setCvar("ui_BrandColorSecondary", level.ui_BrandColorSecondary);
	makeCvarServerInfo("ui_BrandColorSecondary", "1");

	level.ui_BrandColorTertiary = getCvar("ui_BrandColorTertiary");
	if(level.ui_BrandColorTertiary == "")
		level.ui_BrandColorTertiary = "2";
	setCvar("ui_BrandColorTertiary", level.ui_BrandColorTertiary);
	makeCvarServerInfo("ui_BrandColorTertiary", "2");

	level.ui_BrandNextMap = getCvar("ui_BrandNextMap");
	if(level.ui_BrandNextMap == "")
		level.ui_BrandNextMap = "^1~^3empire ^2| ^1Next:";
	setCvar("ui_BrandNextMap", level.ui_BrandNextMap);
	makeCvarServerInfo("ui_BrandNextMap", "^1~^3empire ^2| ^1Next:");
	...
	if(level.ui_AutoAdmin_AFK_NotifyActionTaken == "")
		level.ui_AutoAdmin_AFK_NotifyActionTaken = "^1~^3empire ^2| ^1automod: ^3You have been forced into spectator mode due to ^1AFK^3 behavior.";
	...
	makeCvarServerInfo("g_allowedGametypeVote", "dm tdm sd");
}
...
//added 1903a3
	level.allow_m1903a3 = getCvar("scr_allow_m1903a3");
	if(level.allow_m1903a3 == "")
		level.allow_m1903a3 = "1";
	setCvar("scr_allow_m1903a3", level.allow_m1903a3);
	setCvar("ui_allow_m1903a3", level.allow_m1903a3);
	makeCvarServerInfo("ui_allow_m1903a3", "1");
...
updateGlobalCvars()
{
	for(;;)
	{
		...
		// Added new global cvars for PAM UO Auto-Ready, Server Branding/Customization, and AutoAdmin
		if(level.g_AutoReadyTime != getCvar("g_AutoReadyTime")) {
			level.g_AutoReadyTime = getCvar("g_AutoReadyTime");
			setCvar("ui_g_AutoReadyTime", level.g_AutoReadyTime);
		}

		if(level.g_AutoSpectatorTime != getCvar("g_AutoSpectatorTime")) {
			level.g_AutoSpectatorTime = getCvar("g_AutoSpectatorTime");
			setCvar("ui_g_AutoSpectatorTime", level.g_AutoSpectatorTime);
		}

		if(level.ui_BrandServerName != getCvar("ui_BrandServerName")) {
			level.ui_BrandServerName = getCvar("ui_BrandServerName");
			setCvar("ui_BrandServerName", level.ui_BrandServerName);
		}

		if(level.ui_BrandColorPrimary != getCvar("ui_BrandColorPrimary")) {
			level.ui_BrandColorPrimary = getCvar("ui_BrandColorPrimary");
			setCvar("ui_BrandColorPrimary", level.ui_BrandColorPrimary);
		}

		if(level.ui_BrandColorSecondary != getCvar("ui_BrandColorSecondary")) {
			level.ui_BrandColorSecondary = getCvar("ui_BrandColorSecondary");
			setCvar("ui_BrandColorSecondary", level.ui_BrandColorSecondary);
		}

		if(level.ui_BrandColorTertiary != getCvar("ui_BrandColorTertiary")) {
			level.ui_BrandColorTertiary = getCvar("ui_BrandColorTertiary");
			setCvar("ui_BrandColorTertiary", level.ui_BrandColorTertiary);
		}

		if(level.ui_BrandNextMap != getCvar("ui_BrandNextMap")) {
			level.ui_BrandNextMap = getCvar("ui_BrandNextMap");
			setCvar("ui_BrandNextMap", level.ui_BrandNextMap);
		}
		...
		if(level.allowedGametypeVote != allowedGametypeVote)
		{
			level.allowedGametypeVote = allowedGametypeVote;
			// Logic to handle gametype filtering can be added here
		}
...
//added 1903a3
			case "m1903a3_mp":
				if(!getcvar("scr_allow_m1903a3"))
					{
					self iprintln("M1903-A3 is a restricted weapon");
					response = "restricted";
				}
				break;	
